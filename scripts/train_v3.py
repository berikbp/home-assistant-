#!/usr/bin/env python3
"""Train SAC + PPO fresh, compare both against baseline."""
import sys, os
sys.path.insert(0, os.path.dirname(__file__))

from rl_platform import (load_weather, SimpleHVACEnv, SAC, PPO, CB, State,
...
def train_sac():
    ...
    st = State()
    cb = CB(st)
    print("Training SAC 30k steps (LR=3e-4, buffer=200k)...")
    ...
    save_agent("sac_30k_fresh", "SAC", m, metrics,
...
def train_ppo():
    ...
    st = State()
    cb = CB(st)
    print("Training PPO 30k steps (LR=3e-4)...")
    m = PPO("MlpPolicy", env, learning_rate=3e-4, n_steps=512,
    ...
    metrics = {"episode_rewards": rw, "episode_comforts": cf, "episode_energies": en,
               ...
               "avg_comfort": sum(cf)/len(cf) if cf else 0,
               "avg_energy": sum(en)/len(en) if en else 0}
    save_agent("ppo_30k_fresh", "PPO", m, metrics,
               {"algo":"PPO","timesteps":30000,"energy_weight":0.6,"lr":3e-4,
                "timestep_min":5,"gamma":0.99})
    return m
...
for name, model in [("SAC", sac_model), ("PPO", ppo_model)]:
