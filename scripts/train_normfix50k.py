#!/usr/bin/env python3
"""Fresh SAC 50k with fixed normalization + lower LR."""
import sys, os
...
print("Training SAC 50k steps (fixed norm, LR=1e-4)...")
m = SAC("MlpPolicy", env, learning_rate=1e-4, buffer_size=100000,
        batch_size=256, gamma=0.99, tau=0.005, verbose=1)
m.learn(total_timesteps=50000, callback=cb)
env.close()
print("Training done.")
...
save_agent("sac_50k_normfix_lr1e4", "SAC", m, metrics,
           {"algo":"SAC","timesteps":50000,"energy_weight":0.6,"lr":1e-4,
...
print()
print("=" * 60)
print(f"  SAC 50k (fixed norm, LR=1e-4) vs Baseline")
print("=" * 60)
