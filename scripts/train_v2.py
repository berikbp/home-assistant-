#!/usr/bin/env python3
"""Fresh SAC: lower LR + higher energy weight to beat baseline everywhere."""
import sys, os
...
print("Training SAC 30k steps (LR=1e-5, ew=0.6, buffer=100k)...")
m = SAC(
    "MlpPolicy", env,
    learning_rate=1e-5,
    buffer_size=100000,
...
train_cfg = {"algo": "SAC", "timesteps": 30000, "energy_weight": 0.6, "lr": 1e-5,
             "timestep_min": 5, "gamma": 0.99, "tau": 0.005, "buffer_size": 100000, "batch_size": 256}
meta = save_agent("sac_30k_v3", "SAC", m, metrics, train_cfg)
print(f"Saved: {meta['name']}")

# Evaluate with ew=0.6 (same as baseline comparison)
...
b = bl_m['comfort_score']
r = rl_m['comfort_score']
...
b = bl_m['total_discomfort']
r = rl_m['total_discomfort']
